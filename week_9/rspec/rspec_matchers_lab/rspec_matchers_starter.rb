...
describe "show the depths of rspec" do
  ...
  context "work through the different types of Rspec matchers" do
    ...
    expect(e).to be > d  #   expect(e).to # check if e is greater than d
    expect(e).to be <= f #   expect(e).to # check to see if equal to or less than f
    expect(f).to be >= e #   expect(f).to # check to see if f is greater than or equal to e
    expect(e).to be_between(d, f)#   expect(e).to # check whether e is between d & f
    expect(d).not_to be_between(e, f)#   expect(d).to # check whether d is NOT between e & f
      end

      it 'should test the various class type matchers' do
        g = 5.0
        h = Hash.new
    expect(g).to be_instance_of(Float)#     expect(g).to #check if g is a float
    expect(h).to be_instance_of(Hash)#     expect(h).to #check if h is a hash
      end
